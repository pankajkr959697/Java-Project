...
<html>
 ...
 <h1 class="text-success text-center ">This is Admin Home Page</h1>
 ...
 db.disconnect();
 for(HashMap<String,String> companyDetials:companies){
	 %>
	 <div class="bg-light">
	 <p> Name:<%=companyDetials.get("name") %>
	  <p>Email:<%=companyDetials.get("email") %>
	   <p>Contach No:<%=companyDetials.get("phone") %>
	    <p>Reg_No<%=companyDetials.get("reg_no") %>
	     <p>Address<%=companyDetials.get("address") %></br>
	     <a href='ChangeCompanyStatus?cemail=<%=companyDetials.get("email") %>&status=activate' class='btn btn-success'>Active</a>
	     <a href='ChangeCompanyStatus?cemail=<%=companyDetials.get("email") %>&status=deactivate' class='btn btn-danger'>DeActivate</a>
